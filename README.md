...
The script looks for all files with the name <filename.out> in <path_to_directory>, extracts the data that was calculated at the specified <temperature> (in K), and creates a csv file that contains the extracted data. One row for each file. If <temperature> is not specified, the Orca default temperature - 298.15 K - is used.
